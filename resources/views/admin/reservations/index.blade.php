...
<head>
    ...
    <title>Kelola Reservasi - SportVenue Admin</title>
    <style>
...
<body>
        ...
        <div class="card">
            ...
            <table class="table">
                ...
                <tbody>
                    ...
                    <tr>
                        <td>{{ $reservation->kode_booking_232112 }}</td>
                        <td>{{ $reservation->lapangan->nama_lapangan_232112 ?? 'N/A' }}</td>
                        <td>{{ \Carbon\Carbon::parse($reservation->tanggal_reservasi_232112)->format('d/m/Y') }}</td>
                        <td>{{ $reservation->waktu_mulai_232112 }} - {{ $reservation->waktu_selesai_232112 }}</td>
                        <td>{{ $reservation->lama_pemakaian_232112 }} jam</td>
                        <td>Rp {{ number_format($reservation->total_biaya_232112, 0, ',', '.') }}</td>
                        <td>
                            <span class="status status-{{ $reservation->status_232112 }}">
                                {{ ucfirst($reservation->status_232112) }}
                            </span>
                        </td>
                        <td>
                            @if($reservation->status_232112 == 'pending')
                                <button class="btn btn-success btn-sm" onclick="updateStatus({{ $reservation->reservasi_id_232112 }}, 'confirmed')">Konfirmasi</button>
                                <button class="btn btn-danger btn-sm" onclick="updateStatus({{ $reservation->reservasi_id_232112 }}, 'cancelled')">Tolak</button>
                            @elseif($reservation->status_232112 == 'confirmed')
                                <button class="btn btn-warning btn-sm" onclick="updateStatus({{ $reservation->reservasi_id_232112 }}, 'completed')">Selesai</button>
                                <button class="btn btn-danger btn-sm" onclick="updateStatus({{ $reservation->reservasi_id_232112 }}, 'cancelled')">Batal</button>
                            @endif
                        </td>
                    </tr>
                    @empty
    ...
    <script>
        function updateStatus(reservationId, status) {
            if (!confirm(`Apakah Anda yakin ingin ${status === 'confirmed' ? 'mengkonfirmasi' : status === 'cancelled' ? 'membatalkan' : 'menyelesaikan'} reservasi ini?`)) {
                return;
            }

            fetch(`{{ url('/admin/reservations') }}/${reservationId}/status`, {
                method: 'POST',
                headers: {
